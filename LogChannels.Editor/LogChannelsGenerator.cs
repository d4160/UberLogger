...
public class LogChannelsGenerator
{
    ...
    private static string unityMethodCallTemplateWithContext =
        "namespace {0}\n" +
        "{{\n" +
        "    public partial class {1}\n" +
        "    {{\n" +
        "        [{2}]\n" +
        "        public static void {3}(string message, UnityEngine.Object context)\n" +
        "        {{\n" +
        "            {4}(message, context);\n" +
        "        }}\n" +
        "    }}\n" +
    ...
    private static string uberLoggerMethodCallTemplateWithContext =
        "namespace {0}\n" +
        "{{\n" +
        "    public partial class {1}\n" +
        "    {{\n" +
        "        [{2}]\n" +
        "        [UberLogger.StackTraceIgnore]\n" +
        "        public static void {3}(UnityEngine.Object context, string message)\n" +
        "        {{\n" +
        "            {4}(context, \"{1}\", message);\n" +
        "        }}\n" +
        "    }}\n" +
    ...
    public static string Generate(LogChannelsConfig.Logger outputAPI, string nameSpace, string conditionals, string channel, LogChannelsConfig.Severity severity)
    {
        ...
        /* Context version */
        if (outputAPI == LogChannelsConfig.Logger.Unity)
            formattedTemplate += string.Format(unityMethodCallTemplateWithContext, nameSpace, channel, conditionals, severityStrings.CallMethod, severityStrings.LogMethod);
        else
            formattedTemplate += string.Format(uberLoggerMethodCallTemplateWithContext, nameSpace, channel, conditionals, severityStrings.CallMethod, severityStrings.LogMethod);
